(function(w){
	...
	var lengths =Math.ceil(data.length/as.length);
	var divL,divT;//记录div的left,top
...
/*----------*/
	...
	function btnClick(){
		...
		function fnPrev(){
			...
			wss_canvas(zbjCxt,zbjArrLise,defineArr,null,function(){
				...
				imgWrap.innerHTML = '';
				searchN --;
				// searchN = searchN <= 1 ? lengths  : searchN;
				if(searchN < 1){
					searchN = lengths - 1;
				}
		...
		function fnNext(){
			...
			wss_canvas(zbjCxt,zbjArrLise,defineArr,null,function(){
				zbjListA.innerHTML = '';
				searchN ++;
				// searchN = searchN >= lengths ? 1 : searchN;
				if(searchN > lengths - 1){
					searchN = 1;
				}
				window.location.search = `?list=${searchN}`;
			});
	...
	function getLiSty(search) {
		...
		divT = canvas.style.top = (H-head.offsetHeight-zbjCxt.canvas.height)/2 + 'px';
		divL = canvas.style.left = canvas.offsetLeft + 'px';
	...
	function posA(){
		var zbjlistA = document.querySelector('.zbjlistA');
		zbjlistA.style.width = canvas.width + 'px';
		zbjlistA.style.height = canvas.height + 'px';
		zbjlistA.style.left = divL ;
		zbjlistA.style.top = divT;
		zbjlistA.style.position = 'absolute';
	}
})(window)
